...
object DescriptiveStats {
  ...
  val allContinuousMetrics: List[MetricContinuous] = List(Min, Max, Mean, Count, Variance, Stddev, Sum, Skewness, Kurtosis, Percentile25, Median, Percentile75)
  ...
  def customCountDiscret(dataInit: DataFrame , e: String): DataFrame= {
 ...
 def customFrequencies(dataInit: DataFrame , e: String): DataFrame= {
   val subColFrame: DataFrame = customCountDiscret(dataInit, e)
 ...
 object CountDiscrete extends DiscreteMetric("CountDiscrete", customCountDiscret)
 ...
 val allDiscreteMetrics: List[MetricDiscrete] = List(Category, CountDiscrete, Frequencies)
